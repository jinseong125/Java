 ...
 * java.io.Reader
 * 
 * 1. 문자(Charactor) 기반 입력 스트림의 최상위 추상 클래스입니다.
 ...
 *    3) int read(char[] cbuf, int off, int len) : 읽은 내용을 문자 배열 일부에 저장(인덱스 off부터 len개부터)
 *                                                 실제로 읽은 문자 수 반환
 *                                                 (파일의 끝에 도달하면 EOF(-1) 반환)
 *                                                 
 */
...
public class Main {
  ...
  public static void fileReader() {
    ...
    try (FileReader in = new FileReader(file)) {
      ...
      //----- 파일을 끝까지 읽으면소 StringBuilder에 저장
  ...
  public static void bufferedReader() {
     ...
     * 
     * 1. 문자 기반 입력 스트림에 버퍼링 기능을 제공하는 보조스트림 클래스입니다.
     * 2. 내부 버퍼(기본8192개 문자)에 데이터를 모아 한 번에 읽어옵니다.
     * 3. 개행(줄바꿈) 문자 단위로 입력 받는 메소드를 제공합니다. (BufferedReader만 사용 가능)
  ...
  public static void inputStringReader() {
    ...
    // try (InputStreamReader in = new InputStreamReader(new FileInputStream(file))) { 한글이 안 깨지지만, 버퍼링이 없어서 성능이 떨어집니다.
