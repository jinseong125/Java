 ...
 *  2. 파일 시스템의 파일이나 디렉터리에 대한 추상화를 통해 쉽게 작업을 처리할 수 있습니다.
 ...
 *    2) Mac/Linux : 슬래시 (/)
 *    3) Windows를 이용한 개발과 Linux를 이용한 배포가 서로 다른 경로 구분자를 필요로 함으로 다음과 같이 해결한다.
 *      (1) File.separator 필드를 활용합니다. (운영체제에 따라서 자동으로 경로 구분자가 선택됩니다.)
 *      (2) Windows에서 개발할때도 슬래시(/)를 사용합니다. (대부분 슬래시, 역슬래시 모두 경로 구분자로 인식하도록 호환성을 제공합니다.)
 * 2. 예약된 디렉터리
 *    1) 현재 디렉터리 : .
 *    2) 상위 디렉터리 : ..
 *    3) 루트 디렉터리 : /
...
public class Main {
  ...
  public static void handling() {
    ...
    if(dir.exists()) {
      dir.delete();
      System.out.println("디렉터리가 삭제 되었습니다.");
    } else {
      System.out.println("존재하지않는 데이터입니다.");
    }
    
    //----- 실습. 현재날짜를 디렉터리 이름으로 사용해서 생성하시오. (예: 20250710)
    File dir2 = new File("C:/" + DateTimeFormatter.ofPattern("yyyy/MM/dd").format(LocalDate.now()));
    if(dir.exists() == false);
    dir2.mkdirs();
  }
  
  public static void deleteDirectoryTree(String pathname) {  // recursive call - 재귀호출
