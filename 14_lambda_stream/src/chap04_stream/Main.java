 ...
 *      (2) list.stream(), Arrays.stream(array), Stream.of(값, ...), Files.lines(path) 등 - 연습
 *    2) 중간 연산
...
public class Main {
  ...
  public static void map() throws Exception {
    ...
    .map(season -> season.toUpperCase())   //----------------- 2. 중간 연산 String::toUpperCase
    .forEach(season -> System.out.println(season));  // ------ 3. 최종 연산  System.out::println
  ...
  public static void collect() throws Exception {
    ...
    //----- Stream 생성
    IntStream s = IntStream.range(1, 11);   // 1 이상 11 미만 포함 x (1 ~ 10) 어떤 범위안의 숫자 구하는건 range

    //----- filter()
    IntStream s2 = s.filter(number -> number % 2 == 1);     // 홀수만 구하기 (필터링)
  ...
  public static void listStream() throws Exception {
    ...
    fruits.add(new Fruit("Mango", 300.0));
    fruits.add(new Fruit("Peach", 500.0));
    fruits.add(new Fruit("Kiwi" , 20.0));
    fruits.add(new Fruit("Melon", 40.0));
