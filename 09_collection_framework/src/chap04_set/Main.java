 ...
 * 4. 데이터를 특정한 규칙에 따라 다른값(숫자 등)으로 바꾸는 것.
 * 5. 데이터가 많을 때 빠르게 저장 / 찾기 위해 사용함.
 * 
 * 해시 테이블 (Hash Table)
 * 
 ...
 * 
 * - 해시 함수 (Key를 아용해서 어떤 값을 반환하는 함수)
 *   : 이름의 획수를 해시값으로 사용하자.
 *   
 * - 뽀로로의 해시값은 24 -> 인덱스 24에 뽀로로 객체를 저장
 * - 루피의   해시값은 12 -> 인덱스 12에 루피 객체를 저장
 * - 에디의   해시값은 8  -> 인덱스 8에 에디 객체를 저장
 * - 포비의   해시값은 9  -> 인덱스 9에 포비 객체를 저장
 * - 크롱의   해시값은 12 -> 인덱스 12에 크롱의 객체를 저장 (해시값 충돌 : 체이닝 방식(연결 리스트)등으로 기존 루피 객체에 이어서 크롱 객체를 저장
 * 
 */
