...
public class Main {
	...
	public static void main(String[] args) {
	  ...
	  chihuahua.bark(); // 호출할 땐 Dog의 bark()를 호출하지만, 실행할 땐 Chihuahua의 bark()가 실행됩니다.
	  					...
	  					// 오버라이드된 메소드의 경우 명시적으로 작성된 타입(Dog)이 아닌
	  				    // 실제저장된 객체의 타입(Chihuahua)으로 메소드가 연결되면서(Binding) 실행됩니다.
	                    // 명시적으로 작성된 타입과 실행하는 타입이 바뀌는 것을 "동적바인딩(Dynamic Binding")이라고 합니다.
	  ...
	  dogs[1] = new Husky();
	  for(Dog dog : dogs) {  // 이건 향상된 for문(Enhanced for-loop)
		  					 //	배열이나 컬렉션에 있는 요소를 하나씩 꺼내서 dog 변수에 담는 거야.
		  if (dog != null) {
			  dog.bark();
		  }
	  }
